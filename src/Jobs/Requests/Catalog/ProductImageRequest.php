...
class ProductImageRequest implements ShouldQueue
{
     ...
     * @var string
     */
    private $endpoint = "v3/catalog/products/{product_id}/images";
    ...
    public function handle()
    {
        ...
        if (! is_null($pagination)) {
            if ($pagination->hasMorePages()) {
                ...
                dispatch(new ProductImageRequest($this->productId, $newQuery));
            }
        }
    }
    ...
    {
        ProductImage::updateOrCreate([ 'id' => $item->id ],
            [
                'product_id'   => $this->productId,
                'is_thumbnail' => (bool) $item->is_thumbnail,
                'sort_order'   => $item->sort_order,
                'description'  => $item->description,
                'url_standard' => $item->url_standard,
            ]
        );
    }
}
